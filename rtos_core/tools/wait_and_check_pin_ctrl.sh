...
ssh -o StrictHostKeyChecking=no root@$DEVICE_IP 'echo === ps ===; ps | grep -E "mujina|rtos"; echo === ipc6.log HEAD (boot/enum sequence) ===; head -n 60 /sharefs/ipc6.log; echo === ipc6.log TAIL ===; tail -n 20 /sharefs/ipc6.log; echo === mujina-minerd log tail ===; tail -n 30 /data/mujina-minerd.log 2>/dev/null'
