...
///
/// V0 marks errors with a `0xFF` first data byte, but that byte
/// position also holds normal payload data, so error detection is
/// not reliable at the framing level.
///
/// ## V1
///
/// ```text
...
impl Response {
    ...
    /// stripped).
    ///
    /// All responses are treated as success: a `0xFF` first data byte
    /// would mark an error, but that position also holds normal
    /// payload data, so it is not checked here.
