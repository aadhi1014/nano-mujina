...
pub enum ClientCommand {
    ...
    /// Suggest a new difficulty to the pool. Fractional below 1, integer at or above 1.
...
/// Parse a block hash from a Stratum hex string.
///
/// Stratum v1 encodes the hash as 8 little-endian 4-byte words, each word's
/// bytes written in big-endian hex order ("word-swapped"). This differs from
/// both the big-endian display format and the little-endian internal byte
/// array used by Bitcoin's wire protocol.
///
/// ## Conversion
///
/// 1. Hex decode the string (32 bytes)
/// 2. Split into 8 chunks of 4 bytes each
/// 3. Reverse the bytes within each chunk
/// 4. The result is the internal byte array
///
/// ```text
...
fn parse_block_hash(hex: &str) -> Result<BlockHash, String> {
    ...
    // Reverse bytes within each 4-byte word (word-swap).
...
/// - Notifications (have method and params, no id, or `id: null`)
...
mod tests {
    ...
    fn test_parse_malformed_error_response() {
        // id=null doesn't match the Response variant (expects id: u64)
    ...
    /// Parses a mining.notify JSON payload and checks the resulting fields.
    #[test]
    ...
    /// Checks that SubmitParams::to_stratum_json() produces the expected JSON array.
    #[test]
    fn test_submit_params_serialization_matches_capture() {
        ...
        let params = SubmitParams {
            username: "bc1q...bitaxe".to_string(),
