...
impl StratumV1Client {
    ...
    ///
    /// Sends `mining.subscribe` and waits for a response containing
    ...
    ///
    /// Sends `mining.authorize` with username and password.
    ...
    /// rather than a notification. Pools that honor it reply indirectly via
    /// `mining.set_difficulty` notifications, not by responding to the
    /// request id, so the 3-second timeout is expected to expire for most
    /// pools. While waiting, `send_request`'s internal loop processes any
    ...
    async fn suggest_difficulty(
        ...
        // Values >= 1 are sent as integers; values below 1 are sent as a fraction.
    ...
    pub(crate) async fn run_with_transport(
        ...
        // Suggest difficulty after authorize.
...
mod tests {
    ...
    /// # Test Strategy
    ///
    /// Connects to a pool and runs the handshake sequence:
    ///
    /// 1. **Connect**: Establish TCP connection and spawn client task
    /// 2. **Collect events**: Wait for subscription, job, and difficulty events
    /// 3. **Stability check**: Wait 5 seconds to confirm the pool doesn't disconnect
    ...
    async fn test_submit_share_rejected_with_false_result() {
        ...
        tokio::spawn(async move {
            ...
            // Response with a false result instead of an error
            let response = JsonRpcMessage::Response {
                id: msg.id().unwrap(),
