...
/// Process-wide pool accept/reject counters, for the dashboard's Info page.
/// Not persisted across restarts.
static SHARES_ACCEPTED: AtomicU64 = AtomicU64::new(0);
static SHARES_REJECTED: AtomicU64 = AtomicU64::new(0);

/// Pool accept/reject totals since this process started.
pub fn share_accept_reject_counts() -> (u64, u64) {
    (
...
/// costs at most one message per interval.
...
struct ExponentialBackoff {
    ...
    initial: Duration,
    max: Duration,
    // Per-process jitter seed, seeded from OS randomness at construction.
...
impl StratumV1Source {
    ...
    fn share_to_submit_params(&self, share: Share) -> Result<crate::stratum_v1::SubmitParams> {
        ...
        Ok(crate::stratum_v1::SubmitParams {
            ...
            extranonce2,
            ntime: share.time,
            // The server parses the submitted hex nonce as an integer, then
            // re-encodes it little-endian into the block header. Submitting
            // the byte-swapped value here makes that round trip land on the
            // header bytes our own hash verification expects.
            nonce: share.nonce.swap_bytes(),
            version_bits,
        })
    }
    ...
    ///
    /// Returns `None` for zero hashrate. The value may be fractional below 1;
    /// the client encodes whole difficulties as integers on the wire.
    ...
    ///
    /// Compares `new_diff` against the last suggested value (not the pool's
    /// live set difficulty) using `DOWN_FACTOR`/`UP_FACTOR` as an asymmetric
    /// deadband. Any value is material when there is no prior suggestion.
...
mod tests {
    ...
    /// Converts a captured mining.notify payload into a JobTemplate and
    /// checks the resulting fields.
    #[test]
    ...
    /// Converts a captured Share back to Stratum format and checks it
    /// against the captured mining.submit.
    #[test]
    ...
    /// Checks the Share -> SubmitParams -> JSON round trip against the
    /// captured mining.submit.
    #[test]
