            ...
            Some(cmd) = cmd_rx.recv() => {
                match cmd {
                    ThreadCommand::Configure => {
                        // Placeholder hashrate estimate for a single chip
                                    ...
                                    match task.en2.as_ref().and_then(|en2| template.compute_merkle_root(en2).ok()) {
                                        Some(merkle_root) => {
                                            ...
                                            if task.share_target.is_met_by(hash) {
                                                // Use the harder of the ASIC ticket mask and
                                                // the scheduler target as the effective filter
...
mod tests {
    ...
    fn test_pll_calculations_match_reference() {
        // (freq_mhz, expected_flag, expected_fb_div, expected_ref_div, expected_post_div)
        let test_cases = vec![
