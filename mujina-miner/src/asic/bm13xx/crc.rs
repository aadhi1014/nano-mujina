...
/// Computes a 5-bit CRC-5-USB checksum (polynomial 0x05, init 0x1f, no xorout, no reflection).
...
/// Returns true if the CRC-5 over `data` (data plus trailing CRC byte) is zero.
...
/// Computes a 16-bit CRC-16-CCITT-FALSE checksum (polynomial 0x1021, init 0xFFFF, no xorout, no reflection).
...
mod tests {
    ...
    // Checks that crc5() over each frame's payload (skipping the 2-byte prefix)
    // matches the frame's trailing CRC byte.
