...
impl ChipType {
    ...
    pub fn core_count(&self) -> Option<u32> {
        match self {
            Self::BM1370 => Some(2048), // 128 domains x 16 engines
            _ => None,
        }
    }
}
...
///
/// Stored as the raw 4-byte wire value rather than a parsed u32.
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl NonceRangeConfig {
    // Nonce range values for different chain lengths
    ...
    const S21_PRO: [u8; 4] = [0x00, 0x00, 0x1e, 0xb5]; // 65-128 chips
    ...
    fn encode(&self, dst: &mut BytesMut) {
        match self {
            ...
            Command::ChainInactive => {
                ...
                // Length field includes everything after the preamble except itself
...
mod init_tests {
    ...
    fn pll_calculation_produces_valid_frequencies() {
        ...
        let test_cases = vec![
            ...
            (500.0, [0xa2, 0x02, 0x30]), // 500.00MHz -> 506.25MHz actual
        ];
...
impl BM13xxProtocol {
    ...
    /// Get the initialization sequence for a single chip.
    ///
