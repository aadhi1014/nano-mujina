...
/// GPIO command indices used by the board firmware's control protocol.
/// These are not physical pin numbers; the firmware maps each index
/// to a GPIO internally.
mod gpio_cmd {
...
async fn create_from_usb(device: UsbDeviceInfo) -> Result<BackplaneConnector> {
    ...
    // Allow the VDDIO supply to settle before use.
    ...
    // Calibration offset applied to the remote diode reading.
