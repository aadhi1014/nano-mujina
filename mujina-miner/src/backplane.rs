...
impl Backplane {
    ...
    pub async fn run(&mut self) -> Result<()> {
        // The nano3s board has no transport that emits a TransportEvent, so it
        // is created directly here at startup instead of through
        // handle_cpu_event(). create_nano3s_board() gates on
        // MUJINA_NANO3S_ENABLE internally and returns Err (not panic) when
        // unset, so this is a no-op when the env var isn't set.
        if let Some(descriptor) = self.virtual_registry.find("nano3s") {
            match (descriptor.create_fn)().await {
                Ok(conn) => {
                    let board_id = conn
                        .info
                        .serial_number
                        .clone()
                        .unwrap_or_else(|| "nano3s-0".to_string());
                    self.start_board(board_id, conn).await;
                }
                Err(e) => {
                    info!(board = descriptor.name, error = %e, "nano3s board not started");
                }
            }
        }
